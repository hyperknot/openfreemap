...
PMTILES_PATH = f'{tilegen_deploy_config.pmtiles_bin}/pmtiles'
...
def install_pmtiles(c):
    ...
    c.sudo(f'rm -rf {tilegen_deploy_config.pmtiles_bin}')
    c.sudo(f'mkdir -p {tilegen_deploy_config.pmtiles_bin}')
    ...
    sudo_cmd(
        ...
        f'tar -xzf /tmp/go-pmtiles.tar.gz -C {tilegen_deploy_config.pmtiles_bin} pmtiles && '
