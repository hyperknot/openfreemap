...
GO_PMTILES_RELEASE = '1.31.2'
PMTILES_PATH = f'{tilegen_deploy_config.remote_pmtiles_bin}/pmtiles'


def install_pmtiles(c: Connection) -> None:
    # Normal deploys preserve this selection; repository pin changes apply only
    # after the explicit destructive --reinstall removes the installed binary.
    ...
    sudo_cmd(
        ...
        + f'chown root:root {PMTILES_PATH} && chmod +x {PMTILES_PATH} && '
