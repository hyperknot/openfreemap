...
The equivalent manual deploy command is:

```bash
cd /data/ofm/src && sudo -u ofm env PYTHONUNBUFFERED=1 ./tilegen/scripts/tilegen.py set-version planet
```
...
linux_host servers use this deployed version file to decide which planet version to download and mount.
