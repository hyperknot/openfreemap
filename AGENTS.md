...
- `linux_host/` — linux_host runtime code.
  - Script: `linux_host/scripts/linux_host.py`
  - Runtime library: `linux_host/linux_host_lib/`
  - Cron file: `linux_host/cron.d/ofm_linux_host`
- `tilegen/` — tilegen runtime code.
...
- `/data/ofm/config/linux_host` — selected linux_host runtime config.
- `/data/ofm/config/tilegen` — selected tilegen runtime config.
- `/data/ofm/linux_host` — linux_host runtime data, logs, assets and runs.
- `/data/ofm/tilegen` — tilegen runtime data, logs, runs and Planetiler.
...
```bash
cd /data/ofm/src && sudo env PYTHONUNBUFFERED=1 ./linux_host/scripts/linux_host.py sync
